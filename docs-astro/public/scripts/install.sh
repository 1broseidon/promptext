...
Usage: bash [options] (when piped from curl)
   or: $0 [options] (when running locally)
...
Examples:
    ...
    curl -sSL <url> | bash                              # Install to ~/.local/bin
    curl -sSL <url> | bash -s -- --dir ~/bin           # Install to custom directory
    curl -sSL <url> | bash -s -- --uninstall           # Uninstall promptext

    # When running locally:
    ./install.sh                                        # Install to ~/.local/bin
    ./install.sh --dir ~/bin                           # Install to custom directory
    ./install.sh --uninstall                           # Uninstall promptext
...
install_promptext() {
    ...
    echo "  curl -sSL chain.sh/promptext/scripts/uninstall.sh | bash"
}
