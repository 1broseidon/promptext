...
### Added
- Homebrew Formula support via `brew install 1broseidon/tap/promptext`

### Changed
- Replace O(n²) bubble sort with `sort.Slice` in relevance scorer
- Explicit error ignores (`_ =`) for cleanup `os.Remove` calls in updater
- Fix stale build comment referencing removed `commit` ldflag
...
### Changed
- **README Polish**: Improved clarity and technical precision
- Better explanation of PTX format benefits
